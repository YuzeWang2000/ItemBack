...
- 空间或容器只要仍有未归档直接子节点，归档就返回 409；把容器改成普通物品使用相同保护。
- 移动节点更新和 `Movement` 创建位于同一事务中。

应用不向用户提供物理删除物品的入口。附件删除是明确操作，会先删除数据库记录，再尝试清理文件；清理异常进入服务端日志。上传先写入安全文件，再用事务创建所有记录；数据库失败会清理本批文件。
...
缺少入手日期时两个派生值均为空；有日期但缺少价值时只返回持有天数，日均成本为空。结束日期不得早于入手日期。
...
搜索把连续空白规范成单个空格、限制 100 字符，按名称、品牌、型号、序列号和描述进行大小写不敏感匹配，并返回分页信息和每项完整路径。

## 测试边界

- 单元：日期边界、当天、结束日、缺金额/日期、零金额、非法父节点、自身/后代移动、非空归档、存储 key 路径穿越。
- 集成：用原生 PostgreSQL 迁移后通过 Supertest 验证认证、创建、嵌套、成本、循环拒绝、移动、搜索、统计、多附件和主动内容下载。
